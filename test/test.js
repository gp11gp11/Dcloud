const Dcloud = artifacts.require('./Dcloud.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('Dcloud', ([deployer, uploader]) => {
  let Dcloud

  before(async () => {
    Dcloud = await Dcloud.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await Dcloud.address
    ...
    it('has a name', async () => {
      const name = await Dcloud.name()
      assert.equal(name, 'Dcloud')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await Dcloud.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await Dcloud.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await Dcloud.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await Dcloud.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await Dcloud.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await Dcloud.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await Dcloud.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await Dcloud.files(fileCount)
